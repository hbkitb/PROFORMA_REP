codeunit 50150 "50150_Diverse_OP_ERPG"
{
    ...
    begin
        ...
        /* 230719 */
        cust.Reset();
        //230321 cust.SetRange("Gen. Bus. Posting Group", '11');
        if cust.FindSet then
            repeat
                if (cust."Customer Posting Group" = 'INDLAND') OR (cust."Customer Posting Group" = 'KONTANT') then begin
                    cust."Gen. Bus. Posting Group" := cust."Customer Posting Group";
                    cust."VAT Bus. Posting Group" := 'INDENLANDS';
                    cust.Modify;
                end;
                if (cust."Customer Posting Group" = 'EXPORT') then begin
                    cust."Gen. Bus. Posting Group" := cust."Customer Posting Group";
                    cust."VAT Bus. Posting Group" := 'UDLAND';
                    cust.Modify;
                end;
                if (cust."Customer Posting Group" = 'EXPORTEF') then begin
                    cust."Gen. Bus. Posting Group" := cust."Customer Posting Group";
                    cust."VAT Bus. Posting Group" := 'EU';
                    cust.Modify;
                end;
            until cust.Next = 0;
        /* 230719 */
        ...
        if vend.FindSet then
            repeat
                if (vend."vendor Posting Group" = 'STANDARD') then begin
                    vend."Gen. Bus. Posting Group" := vend."vendor Posting Group";
                    vend."VAT Bus. Posting Group" := 'INDENLANDS';
                    vend.Modify;
                end;
                if (vend."vendor Posting Group" = 'IMPORT') then begin
                    vend."Gen. Bus. Posting Group" := vend."vendor Posting Group";
                    vend."VAT Bus. Posting Group" := 'UDLAND';
                    vend.Modify;
                end;
                if (vend."vendor Posting Group" = 'IMPORT-EF') then begin
                    vend."Gen. Bus. Posting Group" := vend."vendor Posting Group";
                    vend."VAT Bus. Posting Group" := 'EU';
                    vend.Modify;
                end;
